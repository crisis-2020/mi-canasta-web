<template>
  <div class="stock-container">
    <div class="field-container margin">
      <h1 class="stock-container__header h1">Stock</h1>
...
export default {
  ...
  methods: {
    isAdmin() {
      for (
        let i = 0;
        i < AuthService.getUsuarioAutenticacion().usuario.roles.length;
        i++
      ) {
        if (
          AuthService.getUsuarioAutenticacion().usuario.roles[i]
            .rolPerfilId == 3
        ) {
    ...
    async getStocksFirst() {
      try {
        const res = await StockService.getStocks(
          AuthService.getUsuarioAutenticacion().usuario.tienda.tiendaId
        );
    ...
    async putStock(i) {
      try {
        if (this.stocks[i].cantidad != this.stocksUpdate[i].cantidad) {
          await TiendaService.putStock(
            this.stocks[i].tiendaId,
            this.stocks[i].productoId,
            { cantidad: parseInt(this.stocksUpdate[i].cantidad) }
          );
        }
      } catch (error) {
        console.log(error);
